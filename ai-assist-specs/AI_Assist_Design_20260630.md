# Design: AI Assist — Platform Build Strategy
...
Identify your WISMO pilot customer (Customer A) by end of next week. Named account, named
contact, verbal deployment commitment. That customer makes every Month 1 architectural
decision real — without them, the template wedge is theoretical.

Then: fill the blocked fields above and schedule the Month 1 architecture session (Weeks 1–4).
Agenda: open questions 1–5, in order, with named owners walking in with prepared proposals.

---

## What I noticed about how you think

- You produced a spec with explicit "out of scope" boundaries. Most internal specs expand
  to fill available space. Yours contracts to what's needed.

- You named the cross-cutting dependencies (Section 4 of the original spec) before the
  individual UCs had a chance to each claim them. "Settle these centrally rather than per
  use case" is exactly how you prevent a platform from becoming five loosely related features.

- You chose Approach B over C without hesitation. Own that explicitly with engineering:
  "We are not doing discovery first. The open questions get answered in Month 1 architecture,
  not by waiting."

- The spec frames Cognigy as invisible to customers but doesn't name the risk: if Cognigy
  changes a core API, you absorb the migration cost. Version-pinning + compatibility test suite
  + written API commitment is the mitigation. Worth leading with when you present to engineering.

---

## Reviewer Concerns

The following issues were flagged across two review iterations and are not resolved in this
document. They represent known gaps to address before detailed engineering begins:

1. **"What I noticed about how you think" and "The Assignment" sections** do not belong in
   a technical design document. They are useful in an office hours context but should be
   moved to a separate communication before this document is distributed to engineering teams.
   Action: extract to a separate memo before distribution.

2. **Premises section partially duplicates Dependencies section**. Premise 3 (PS incentive
   model) covers ground already in Dependencies. Acceptable for now; tighten before
   distributing to engineering.
...
|--------|---------|-----|------|--------|----------|
| CEO Review | `/plan-ceo-review` | Scope & strategy | 1 | issues_open | 5 proposals, 0 accepted, 3 critical gaps |
| Eng Review | `/plan-eng-review` | Architecture & tests (required) | 1 | issues_open | 13 issues, 4 critical gaps |
| Design Review | `/plan-design-review` | UI/UX gaps | 0 | — | — |
| Adversarial | subagent | Spec review loop (CEO plan) | 3 iterations | 8/10 after fixes | 18 found, 16 fixed, 2 Reviewer Concerns |

**ENG REVIEW DECISIONS (2026-07-01):** D1 (RBAC): convention-based Phase 1, API-layer Phase 2. D2 (Audit fail-closed): fail-closed Auto mode, fail-open Suggest→approve. D3 (Hook timeout): deferred to Month 1 architecture session. D4 (Schema CI): breaking-change check added to CI. D5 (Drain window): upgrade applies on next conversation start, 5-min drain window. D6 (Compile-once): compiled graph is canonical; recompile requires re-review. D7 (E2E modes): three autonomy mode E2E suites added as Month 6 Gate 1 criterion. D8 (Kill switch SLA): deferred to Month 1 architecture session. D9 (Latency per-component): deferred to Month 5-6 staging measurement. D10 (Async compile UX): async with 60s timeout and in-page notification.

**CRITICAL GAPS (4):** (1) MCP malformed response — no handler, silent failure [TODO T-E11]. (2) Reconciliation persistent divergence — no stuck-state detection [TODO T-E13]. (3) UI intent brand token missing — silent render failure [TODO T-E12]. (4) UI intent schema version mismatch — silent corruption risk [TODO T-E12].

**P1 TASKS (5 — must resolve before implementation):** T-E1 RBAC trust assumption doc; T-E2 audit fail-closed/open in design doc; T-E5 drain-window spec; T-E6 compile-once spec; T-E8 E2E test suite for all 3 autonomy modes.

**UNRESOLVED (0):** All review decisions resolved. 3 CEO-review gaps remain open (product decisions, not engineering).

**VERDICT:** Eng review complete — 13 issues found, all captured as design-doc update tasks. 4 critical failure-mode gaps accepted as TODOs. Resolve 5 P1 tasks before Month 1-2 implementation begins. CEO review 3 critical gaps still require product decisions.
